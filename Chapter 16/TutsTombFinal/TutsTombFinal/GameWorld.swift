...
class GameWorld : GameObjectNode, SKPhysicsContactDelegate {
    ...
    var gameoverSound = Sound("snd_gameover")
    var treasureCollectSound = Sound("snd_treasure_collected")
    ...
    func didBeginContact(contact: SKPhysicsContact) {
        ...
        if firstBody?.type == secondBody?.type || firstBody?.type == TreasureType.Magic
            || secondBody?.type == TreasureType.Magic {
                ...
                contactPoint.x -= size.width/2
                contactPoint.y -= size.height/2
                addGlittersAroundPosition(contactPoint, number: 15)
