...
class GameWorld : SKNode {
    ...
    func setup() {
        ...
        let scorebar = SKSpriteNode(imageNamed: "spr_scorebar")
        let scorepos = CGPoint(x: -size.width/2 + scorebar.size.width/2 + 10, y: size.height/2 - scorebar.size.height/2 - 10)
        scorebar.position = scorepos
